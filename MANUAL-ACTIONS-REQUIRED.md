...
6. **Confirm the discretion commitment.** The "For planners and venues" section states that we do not name clients, publish photographs, or release recordings from a private engagement without permission, and that the consort will sign a confidentiality agreement where one is required. This is a promise to the reader, not a description of past practice &mdash; confirm you will honour it (including with deputies and instrumentalists) or cut the paragraph before merging.
7. **Optional: bespoke OG image.** The page currently shares `assets/og-image.png`. A bespoke 1200×630 image for private/international engagements would improve link previews; replace the `og:image`/`twitter:image` URLs and alt text on the page when it exists.
