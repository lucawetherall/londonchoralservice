...
- **`videos[*].upload_date`** + **`videos[*].duration`** — five YouTube videos' metadata couldn't be resolved automatically (consent gate on four; YouTube is blocked outright by the egress proxy for the fifth, `mKMjUvCCW3E`). Open each video on YouTube manually, capture the upload date (YYYY-MM-DD) and the duration (convert to ISO 8601, e.g. 3:42 → `PT3M42S`), update the YAML, and rerun the VideoObject sweep on `pricing.html` and `listen.html`. `mKMjUvCCW3E` has no `VideoObject` anywhere yet — it needs one written on `listen.html`, `christmas.html`, and `carol-singers.html`.
...
3. **Carol recordings.** Partly done (August 2026): one Christmas recording — Blue Christmas, `mKMjUvCCW3E` — now leads the "Hear our musicians" sections on christmas.html and carol-singers.html and sits under "Something different" on listen.html. Everything else in those sections is still hymn recordings. Record 1–2 actual carols (even rehearsal-quality video) and hand them to an agent to wire in the same way. The ensemble size for `mKMjUvCCW3E` is labelled "Choir" because nobody could watch it from the build environment — correct it to "Small Choir" / "Full Choir" if that is wrong.
