...
**Goal:** Send qualified international and destination wedding enquiries to `/private-events.html`, which currently receives in-copy links from exactly two pages. Four workstreams: one music-guide (A), one planners-and-venues page (B), eighteen guide cross-links (C), and a `/destinations/` section covering twenty-two countries (D). Metric: cost per qualified enquiry, per page.
...
| **2** | Eleven Europe/short-haul country pages | The rite spine; highest volume; shortest research. |
| **3** | Eleven long-haul country pages | The no-building spine; needs the permit research, the United States especially. |
...
- **New directories, new globs.** `validate_jsonld.py` (lines 10–14) and `validate_house_claims.py` (the `FILES` tuple) both enumerate directories explicitly. Add **both** `destinations/*.html` and `destinations/**/*.html` in PR 1, so the later region pages are covered without a second edit.
...
- [ ] **Step 1:** Create `partials/private-register.css.html` containing `private-events.html` lines 64–609 verbatim — the `<style>` block, **comment line first**, with the comment rewritten to say it now serves the whole private register and to name this spec.
...
- [ ] **Step 1:** `private-events.html:708` — the footprint sentence, currently "Europe, North America, and the Gulf".
- [ ] **Step 2:** `private-events.html:852` — the "Where do you travel?" answer. **Point it at `/destinations/`; do not list twenty-two countries in an FAQ answer.** Update the matching `FAQPage` JSON-LD text verbatim.
...
- [ ] **Step 3 (PR 1):** Draft the eighteen guide cross-link sentences — **each written for the guide it sits in**, not one sentence eighteen times. Model: weddings.html:2404.
...
- [ ] **Step 5:** Confirm `js/private-events.js` is referenced and element IDs match (`pe-enquiry`, `pe-form-success`, `ensemble-size`, `enquiring-as`, `date-flexible`, `hear`). **The JS itself is not edited** — already null-guarded for the optional voicing selector (js/private-events.js:56–64).
...
- [ ] **Step 3:** Add the eighteen per-guide cross-link sentences — to `/destinations/` where the guide's subject suggests a destination, otherwise `/private-events.html`. **Wedding guides only.**
...
- [ ] **Step 3:** `sitemap.xml` — entries for this shipment's pages, **only now that the files exist**; refresh `lastmod` on every page whose copy changed.
- [ ] **Step 4:** `llms.txt` — a `## Destinations` grouping rather than twenty-two loose lines under Main Pages; one line per page in house format.
...
| PR 1 | ~28 files: index, guide, planners page, eighteen guide edits, `music-guides/index.html`, `private-events.html`, two validators, `sitemap.xml`, `llms.txt`, two new partials. |
| PR 2 and PR 3 | ~14 files each: eleven country pages, `sitemap.xml`, `llms.txt`, `destinations/index.html`. |
...
4. **Google Ads:** a dedicated conversion action for the destinations funnel, and paid search on destination terms — the hub already carries a reserved H1 variant for wedding-targeted paid traffic (private-events.html:711–716).
